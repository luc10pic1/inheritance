...
bool Chessboard::checkCoordinates(int x, int y, bool start) const {
    bool result = true;
    if (x > 7 || x < 0 || y > 7 || y < 0) {
...
void Chessboard::printCapture(std::list<Piece *> color) const {
    auto it = color.begin();
...
void Chessboard::substitionPedone(std::list<Piece *> color) {
    bool result = false;
